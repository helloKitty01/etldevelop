...
function RdbmsViewModel() {
  ...
  self.updateHiveDatabases = function(dbs) {
	self.hiveDatabases(dbs);
  };  
  ...
  self.fetchHiveDatabases = function() {
    var request = {
      url: 'http://10.60.1.120:9831/metastore/databases_list',
      dataType:'json',
      type: 'GET',
      success: function(data) {
        self.updateHiveDatabases(data);
      },
      error: error_fn
    };
    $.ajax(request);
