  ...
  <div class="card card-small" style="margin-top: 0">
    ...
    <div class="card-body">
      ...
      <div class="form-actions" style="padding-left: 10px">
		  ...
		  <button type="button" id="rdb_hive_begin" class="btn btn-primary hide" data-bind="click:begin">开始</button>
		  <button type="button" id="rdb_hive_stop" class="btn btn-primary hide" data-bind="click:rdb_hive_stop">停止</button>
      </div>
      </form>
	  <div class="container hide rdb_hive_container">
      ...
      <div class="progress  progress-striped rdb_hive_progress" style="margin-bottom: 9px;" data-bind="css: {'progress-success': viewModel.updateStatus == 'SUCCEEDED', 'progress-info': viewModel.updateStatus == 'RUNNING' || viewModel.updateStatus == 'PREPARE', 'progress-danger': viewModel.updateStatus == 'KILLED'||viewModel.updateStatus == 'FAILED'}">
  ...
  <div class="card card-small" style="margin-top: 0">
    <h1 class="card-heading simple">
      <ul id="breadcrumbs" class="nav nav-pills hueBreadcrumbBar">
        ...
        <li>
          <a href="/metastore/tables/default">Hive ==> Traditional RDB</a>
        </li>
      </ul>
    </h1>
    <div class="card-body">
      <p>
        <ul class="nav nav-pills hive_rdb_ul">
          <li class="active"><a href="#hive_rdb_step0" class="hive_rdb_step">第 1 步：Hive配置</a></li>		  
          <li><a href="#hive_rdb_step1" class="hive_rdb_step">第 2 步：RDB配置</a></li>
        </ul>

        <form action="#" method="POST" id="mainForm" class="form-horizontal">
      <div class="steps">
        <div id="hive_rdb_step0" class="stepDetails">
            <fieldset>
                <div class="alert alert-info"><h3>Hive相关配置</h3>包含Hive表所在的HDFS目录</div>
				<div id="location" class="control-group">
				<label for="table-external_location" class="control-label">External location</label>
				<div class="controls">
					<input name="table-external_location" value="" type='text' id='hive_rdb_hivedir'  class="pathChooser input-xxlarge" placeholder="/user/user_name/data_dir" /><a class="btn fileChooserBtn" href="#" data-filechooser-destination="table-external_location">...</a>
					<span class="help-block">
				...
				<div class="control-group">
					...
					<label for="delimiter" class="control-label">分隔符</label>
							<div class="controls">
							 <select name="delimiter_0" id="hive_rdb_delimiter">
        ...
        <div id="hive_rdb_step1" class="stepDetails hide">
            <fieldset>
				   <div class="alert alert-info"><h3>RDB相关配置</h3>包含RDB数据库和相应的表名</div>
					<div class="control-group">                        
						<label for="table-field_terminator" class="control-label">RDB数据源</label>
						<div class="controls">
							<select name="table-field_terminator_0" id="hive_rdb_rdbdatabase" data-bind="options: viewModel.servers,optionsText: 'nice_name',optionsValue: 'name'">
							</select>
							<span class="help-block">
								对应字段的解释
							</span>
					...
					<div class="control-group">                        
						<label for="table-field_terminator" class="control-label">RDB数据表</label>
						<div class="controls">
							<select name="table-field_terminator_0" id="hive_rdb_rdbtable" data-bind="options: viewModel.databases">
							</select>
							<span class="help-block">
								对应字段的解释
							</span>
      ...
      <div class="form-actions" style="padding-left: 10px">
          <button type="button" id="hive_rdb_backBtn" class="btn hide">返回</button>
          <button type="button" id="hive_rdb_nextBtn" class="btn btn-primary">下一页</button>
		  <button type="button" id="hive_rdb_begin" class="btn btn-primary hide" data-bind="click:begin">开始</button>
		  <button type="button" id="hive_rdb_stop" class="btn btn-primary hide" data-bind="click:stop">停止</button>
      </div>
      </form>
	  <div class="container hide hive_rdb_container">
	    <table class="table table-striped table-condensed datatables tablescroller-disable">
          <thead>
          <tr>
            <th width="20%">Name</th>
      ...
      <div class="progress  progress-striped hive_rdb_progress" style="margin-bottom: 9px;" data-bind="css: {'progress-success': viewModel.updateStatus == 'SUCCEEDED', 'progress-info': viewModel.updateStatus == 'RUNNING' || viewModel.updateStatus == 'PREPARE', 'progress-danger': viewModel.updateStatus == 'KILLED'||viewModel.updateStatus == 'FAILED'}">
        <div class="bar hive_rdb_bar" style="width: 0%"></div>
      </div>
    </div>
      </p>
    </div>
  ...
  <div class="card card-small" style="margin-top: 0">
    ...
    <div class="card-body">
      ...
      <div class="form-actions" style="padding-left: 10px">
		  ...
		  <button type="button" id="rdb_hdfs_begin" class="btn btn-primary hide" data-bind="click:rdb_hdfs_begin">开始</button>
		  <button type="button" id="rdb_hdfs_stop" class="btn btn-primary hide" data-bind="click:rdb_hdfs_stop">停止</button>
      </div>
      </form>
	  <div class="container hide rdb_hdfs_container">
	    <table class="table table-striped table-condensed datatables tablescroller-disable">
          <thead>
          <tr>
            <th width="20%">Name</th>
...
$(document).ready(function () {
  ...
  $(".hive_rdb_step").click(function (event) {	
    event.preventDefault();
    if (validateForm()) {
      $(".stepDetails").hide();
      var _step = $(this).attr("href");
      $(_step).css("visibility", "visible").show();
      $("#backBtn").hide();
      if (_step != "#hive_rdb_step1") {
        $("#hive_rdb_nextBtn").css("visibility", "visible").show();
        $("#hive_rdb_begin").hide();
		$("#hive_rdb_backBtn").hide();	  
      }
      if (_step != "#hive_rdb_step0") {
        $("#hive_rdb_backBtn").css("visibility", "visible").show();
		$("#hive_rdb_begin").css("visibility", "visible").show();
		$("#hive_rdb_nextBtn").hide();
      }
      $(".hive_rdb_step").parent().removeClass("active");
      $(this).parent().addClass("active");
    }
  });
  ...
  $("#hive_rdb_nextBtn").click(function () {
    $("ul.hive_rdb_ul li.active").next().find("a").click();
  });

  $("#hive_rdb_backBtn").click(function () {
...
<script type="text/javascript">
  ...
  ko.applyBindings(viewModel);
	...
	var data={
		rdbdatabase:$('#'+viewModel.prefix()+'_rdbdatabase').val(),
		rdbtable:$('#'+viewModel.prefix()+'_rdbtable').val(),
		hiveDir:$('#'+viewModel.prefix()+'_hivedir').val(),
		delimiter:$('#'+viewModel.prefix()+'_delimiter').val(),
		hivedatabase:$('#'+viewModel.prefix()+'_hivedatabase').val(),
		hivetable:$('#'+viewModel.prefix()+'_hivetable').val()
	}
    var request = {
      url: 'http://10.60.1.149:4567/etl/'+viewModel.prefix()+'?userid=1',
      dataType: 'jsonp',
	  jsonp:'jsonpcallback',
	  jsonpcallback:'skycallback',
      type: 'GET',
      success: function(data) {
	  viewModel.updateJobId(data.logid);
...
function begin(){
	$('#'+viewModel.prefix()+'_begin').attr('disabled',"true");
	$('#'+viewModel.prefix()+'_stop').css("visibility", "visible").show();
	var data={
		rdbdatabase:$('#'+viewModel.prefix()+'_rdbdatabase').val(),
		rdbtable:$('#'+viewModel.prefix()+'_rdbtable').val(),
		rowkey:$('#'+viewModel.prefix()+'_rowkey').val(),
		hbasetable:$('#'+viewModel.prefix()+'_tablename').val(),
		columnfamily:$('#'+viewModel.prefix()+'_columnfamily').val(),
		hiveDir:$('#'+viewModel.prefix()+'_hivedir').val(),
		delimiter:$('#'+viewModel.prefix()+'_delimiter').val(),
		hivedatabase:$('#'+viewModel.prefix()+'_hivedatabase').val(),
		hivetable:$('#'+viewModel.prefix()+'_hivetable').val()
    ...
    var request = {
	  ...
	  viewModel.updateJobId(data.logid);
	  startProcess();
	  $('.'+viewModel.prefix()+'_container').removeClass('hide');
      },
      data: data
    };
    $.ajax(request);
 ...
 function stop(){
	$('#'+viewModel.prefix()+'_stop').hide();
	$('#'+viewModel.prefix()+'_begin').removeAttr("disabled");
    var request = {
      ...
      success: function(data) {
			...
			viewModel.updateStatus('KILLED');
			viewModel.updateName('RDB->Hive');
			viewModel.updatePercentage('100%');
			$('.'+viewModel.prefix()+viewModel.context_bar()).css('width','100%');
			$.jHueNotify.info("成功停止当前ETL任务！");
			$('.'+viewModel.prefix()+viewModel.context_progress()).removeClass('progress-info');
			$('.'+viewModel.prefix()+viewModel.context_progress()).removeClass('progress-success');
			$('.'+viewModel.prefix()+viewModel.context_progress()).addClass('progress-danger');
    ...
    function startProcess() {
		...
		var request = {
		  ...
		  success: function(data) {
			if (data.status === 0) {
				viewModel.updateStatus('PREPARE');
				viewModel.updatePercentage('0%');
			    $('.'+viewModel.prefix()+viewModel.context_bar()).css('width',0);
			} else if (data.status === 1){
			viewModel.updateStatus('RUNNING');
			viewModel.updateName('RDB->Hive');
			viewModel.updatePercentage(data.process+'%');
			$('.'+viewModel.prefix()+viewModel.context_progress()).removeClass('progress-success');
			$('.'+viewModel.prefix()+viewModel.context_progress()).removeClass('progress-danger');
			$('.'+viewModel.prefix()+viewModel.context_progress()).addClass('progress-info');
			$('.'+viewModel.prefix()+viewModel.context_bar()).css('width',viewModel.processPercentage());			
			...
			viewModel.updateStatus('SUCCEEDED');
			viewModel.updateName('RDB->Hive');
			viewModel.updatePercentage('100%');
			$('.'+viewModel.prefix()+viewModel.context_bar()).css('width',viewModel.processPercentage());	
			window.clearInterval(progressInterval);	
			$('.'+viewModel.prefix()+viewModel.context_progress()).removeClass('progress-info');
			$('.'+viewModel.prefix()+viewModel.context_progress()).removeClass('progress-danger');
			$('.'+viewModel.prefix()+viewModel.context_progress()).addClass('progress-success');
			$.jHueNotify.info(viewModel.processName()+":"+viewModel.jobId()+"导出成功！");
	...
	$("#id_to").change(function () {
		...
		if(viewModel.fromSelected()=='Traditional RDB'&&viewModel.toSelected()=='HDFS'){
				$('.rdb_hdfs').removeClass('hide');
				viewModel.prefix('rdb_hdfs');
				viewModel.updateName();
				viewModel.updatePercentage();
				viewModel.updateStatus();
				viewModel.updateJobId();
		...
		if(viewModel.fromSelected()=='Hive'&&viewModel.toSelected()=='Traditional RDB'){
				$('.hive_rdb').removeClass('hide');
				viewModel.prefix('hive_rdb');
				viewModel.updateName();
				viewModel.updatePercentage();
				viewModel.updateStatus();
				viewModel.updateJobId();
